...
我们采用基于重构或预测误差的异常检测范式作为核心机制，并辅以统计检测。这个设计类似于在时间序列异常检测中广泛应用的 **SALAD 模型** (Stochastic Adversarial Learned Anomaly Detection)，该模型也结合了自编码器和对抗网络来更好地建模复杂时序分布，并使用重构误差和判别误差的组合来计算异常分数。SALAD 的最终异常分数 $S$ 定义为重构误差和判别误差的加权和：
$$
