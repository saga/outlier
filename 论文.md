...
我们基于金融领域业务特点，设计了一个结合统计、卷积和循环网络优势的**三阶段多层检测策略**。该策略旨在结合快速筛选（统计/传统机器学习方法）和深度上下文建模（混合深度学习模型）的优势，以提高检测的准确性和对复杂异常模式（如局部异常和片段异常）的敏感性。采用基于**重构/预测误差**的深度学习范式作为核心，辅以**传统邻近度检测**，并通过加权融合机制得出最终的异常分数。
...
首先对两阶段分数进行**归一化**（Min-Max Scaling），使其位于 $[0, 1]$ 范围内，保证融合的公平性。
