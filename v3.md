这是一个完整的 Python 实验代码，实现了之前讨论的**三阶段多层异常检测策略**。该代码集成了数据生成、数据预处理、PyOD 的 Isolation Forest（阶段 1）和 PyTorch 的 CNN-LSTM AutoEncoder（阶段 2），以及分数融合和结果可视化（阶段 3）。

请将以下内容保存为单个 Python 文件，例如 `multi_layer_tsad.py`。
...
1.  **异常类型 (Anomaly Types)**：模拟数据中插入了**点异常**（单点或连续几点的剧烈偏离）和**集合异常**（连续子序列表现出不寻常的模式，例如持续的趋势偏移）。这种多样性确保了多层策略的必要性。
2.  **多层策略**：
    *   **阶段 1 (IForest)**：利用 Isolation Forest 的高效率和适用于高维数据（窗口展平后）的特点，用于快速捕获明显的点异常或情境异常。
    *   **阶段 2 (CNN-LSTM AE)**：AutoEncoder（自编码器）是基于重构误差的预测方法。我们采用 CNN-LSTM 混合架构，旨在结合 **CNN 提取局部特征**的能力和 **LSTM 捕捉时间依赖关系**的能力，这对于识别复杂的**集合异常**（即序列模式的偏离）至关重要。
3.  **分数融合**：最终分数 $S_{final}$ 是通过对 $S_1$ 和 $S_2$ 进行 **Min-Max 归一化**后加权求和得出的，以平衡不同算法对异常的互补视图。
4.  **可视化**：图表展示了原始序列、两个阶段的归一化分数，以及最终融合分数和动态阈值。可视化是深度学习模型可解释性研究的重要方向。在时间序列分析中，折线图、直方图等方法常用于探索性数据分析。图表将直观地展示 **IForest** 对剧烈点异常的敏感性（$S_1$ 分数峰值）和 **AE** 对连续模式变化的敏感性（$S_2$ 分数上升）。